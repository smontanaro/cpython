...
- Assuming you already have a running interpreter, run:
...
Now, add two ``def_op`` calls to the end of Lib/opcode.py::
...
This fails because we haven't implemented anything yet.  To get
farther, we need to implement the relevant functions in
``Lib/rattlesnake``, probably in ``function.py``.

Rest is currently TBD.
