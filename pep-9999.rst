...
- Implemented 44 instructions - BINARY_*_REG, INPLACE_*_REG,
  UNARY_*_REG, RETURN_VALUE_REG, LOAD_*_REG, STORE_FAST_REG,
  COMPARE_OP_REG, JUMP_IF_(TRUE|FALSE)_REG, CALL_FUNCTION_REG,
  CALL_FUNCTION_KW_REG, BUILD_(TUPLE|LIST)_REG, LIST_EXTEND_REG.
...
- Register allocation - A long lost observation by Tim Peters that the
  ...
  allocation.  Stinner's implementation did [#regstin]_.  (hard)
...
- Instruction implementation - Instruction implementation in both the
  translator and ceval.c is incomplete (See Current Status for
  details.)  The PyVM instruction set currently contains 119
  instructions.  Since the PyVM instruction set has grown
  significantly over the past two decades, it makes sense that this be
  tackled by multiple people in parallel.  (easy to medium)
