...
class TestSpecifics(unittest.TestCase):
    ...
    def compile_single(self, source):
        regcompile(source, "<single>", "single")

    def assertInvalidSingle(self, source):
        self.assertRaises(SyntaxError, self.compile_single, source)

    def test_no_ending_newline(self):
        regcompile("hi", "<test>", "exec")
        regcompile("hi\r", "<test>", "exec")

    def test_empty(self):
        regcompile("", "<test>", "exec")

    def test_other_newlines(self):
        regcompile("\r\n", "<test>", "exec")
        regcompile("\r", "<test>", "exec")
        regcompile("hi\r\nstuff\r\ndef f():\n    pass\r", "<test>", "exec")
        regcompile("this_is\rreally_old_mac\rdef f():\n    pass", "<test>", "exec")
...
''' % ((longexpr,)*10)
    ...
    def test_indentation(self):
        # testing regcompile() of indented block w/o trailing newline"
        s = """
if 1:
    if 2:
        pass"""
        regcompile(s, "<string>", "exec")
    ...
    def test_compile_ast(self):
        ...
        for fname, code in sample_code:
            co1 = regcompile(code, '%s1' % fname, 'exec')
            ast = regcompile(code, '%s2' % fname, 'exec', _ast.PyCF_ONLY_AST)
            self.assertTrue(type(ast) == _ast.Module)
            co2 = regcompile(ast, '%s3' % fname, 'exec')
            self.assertEqual(co1, co2)
            # the code object's filename comes from the second compilation step
            self.assertEqual(co2.co_filename, '%s3' % fname)

        # raise exception when node type doesn't match with compile mode
        co1 = regcompile('print(1)', '<string>', 'exec', _ast.PyCF_ONLY_AST)
    ...
    def test_compile_filename(self):
        for filename in 'file.py', b'file.py':
            code = regcompile('pass', filename, 'exec')
    ...
    def test_null_terminated(self):
        ...
        with self.assertRaisesRegex(ValueError, "cannot contain null"):
            regcompile("123\x00", "<dummy>", "eval")
        with self.assertRaisesRegex(ValueError, "cannot contain null"):
            regcompile(memoryview(b"123\x00"), "<dummy>", "eval")
        code = regcompile(memoryview(b"123\x00")[1:-1], "<dummy>", "eval")
        self.assertEqual(eval(code), 23)
        code = regcompile(memoryview(b"1234")[1:-1], "<dummy>", "eval")
        self.assertEqual(eval(code), 23)
        code = regcompile(memoryview(b"$23$")[1:-1], "<dummy>", "eval")
        self.assertEqual(eval(code), 23)
    ...
    def test_merge_constants(self):
        # Issue #25843: regcompile() must merge constants which are equal
    ...
    def test_dont_merge_constants(self):
        # Issue #25843: regcompile() must not merge constants which are equal
    ...
    def test_path_like_objects(self):
        ...
        regcompile("42", FakePath("test_compile_pathlike"), "single")
...
class TestExpressionStackSize(unittest.TestCase):
    ...
    def check_stack_size(self, code):
        ...
        if isinstance(code, str):
            code = regcompile(code, "<foo>", "single")
...
class TestStackSizeStability(unittest.TestCase):
    ...
    def check_stack_size(self, snippet, async_=False):
        def compile_snippet(i):
            ...
            if async_:
                script = "async " + script
            code = regcompile(script, "<script>", "exec")
            exec(code, ns, ns)
