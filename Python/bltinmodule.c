...
/*[clinic input]
regcompile as builtin_regcompile
...
/*[clinic end generated code: output=1c54e65b56049a3f input=4eb73615616baee3]*/
{
    ...
    if (optimize < -1 || optimize > 2) {
        PyErr_SetString(PyExc_ValueError,
                        "regcompile(): invalid optimize value");
        goto error;
    }
    ...
    if (strcmp(mode, "exec") == 0)
        ...
        if (!(flags & PyCF_ONLY_AST)) {
            PyErr_SetString(PyExc_ValueError,
                            "regcompile() mode 'func_type' requires flag PyCF_ONLY_AST");
            goto error;
        }
        compile_mode = 3;
    }
        ...
        const char *msg;
        if (flags & PyCF_ONLY_AST)
            msg = "regcompile() mode must be 'exec', 'eval', 'single' or 'func_type'";
        else
            msg = "regcompile() mode must be 'exec', 'eval' or 'single'";
    ...
    if (is_ast) {
        if (flags & PyCF_ONLY_AST) {
            ...
            result = (PyObject*)_PyAST_CompileObjectR(mod, filename,
                                                     &cf, optimize, arena);
            PyArena_Free(arena);
        }
        goto finally;
